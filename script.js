...
var Diario = (function () {
   ...
   /* var agregarTexto = function (elemento, texto) {
    ...
    var limpiarFormulario = function () {
		...
		$('#boton').html('Agregar').off('click').on('click',crearNoticia);
    	
		$('#titulo, #descripcion, #imagen').val('');
	...
	var dibujarNoticia = function (noticia) {
		...
		botonModificar.appendTo('#' + noticia.id);
		 ...
		 $('<input/>', { type: 'checkbox', value: name }).appendTo('#' + noticia.id);
     ...
     var filtroCategoria = function() {
     	...
     	$('#filtroCategoria').change(function(){     		
   			 ...
   			 for (i = 0; i< noticias.length; i++) { 
	   			 ...
	   			 if(categoriaSeleccionada === noticia.categoria){
   ...
   var borrarMultiple = function(){
   	debugger;

   	$('#borrarSeleccionados').click(function() {

		$('#noticias li input[type="checkbox"]:checked').closest('li').find("button.btn-eliminar").trigger('click');
...
var filtroBusqueda = function (){
		...
		var filterVal = $(this).val();
		 ...
		 for (i = 0; i< noticias.length; i++) { 
	   			...
	   			var noticia = noticias[i]
	   			$('#' + noticia.id).toggle(noticia.titulo.includes(filterVal) === true)

	   			 /*if(noticia.titulo.includes(filterVal) === true){
